...
class Apicontroller extends Controller
{
    ...
    public function getHomeContent(Request $req)
    {
        ...
        $data['courseList'] = Course::where('is_verified',1)->with('teacher')->latest('id')->limit(4)->get();
    ...
    public function saveUserSubscribedCourses(Request $req)
    {
        ...
        if(!$validator->fails()){
            ...
            if($user){
                ...
                $creditlistData = Creditlist::where('user_id',$req->userId)->first();
                if (!$creditlistData) return errorResponse('You do not have any credit now');
                ...
                if($course){
                    ...
                    if($req->whatPurchased == 'chapter'){
                        ...
                        if(!$validator->fails()){
                            ...
                            if($chapter){
                                if ($creditlistData && $creditlistData->amount < $chapter->price) return errorResponse('You do not have enough credit to purchase');
                                ...
                                if(!$checkSubscription){
                                    ...
                                    // creditlist update
                                    $creditlistData->amount = $creditlistData->amount-$chapter->price;
                                    $creditlistData->save();
                        ...
                        foreach ($chapter as $key => $chap) {
                            ...
                            // dd(['credit' => $creditlistData->amount, 'price' => $chap->price]);
                            if ($creditlistData && $creditlistData->amount < $chap->price) return errorResponse('You do not have enough credit to purchase', ['creditlistData' => $creditlistData,'credit' => $creditlistData->amount, 'price' => $chap->price]);
                            if(!$checkSubscription){
                                $newChapterPurchased = new ChapterPurchase();
                                $newChapterPurchased->userId = $user->id;
                                ...
                                // creditlist update
                                $creditlistData->amount = $creditlistData->amount-$chap->price;
                                $creditlistData->save();
                            }
                        }
                        DB::commit();
    ...
    public function getTeacherSlots(Request $req)
    {
        ...
        if(!$validator->fails()){
            ...
            for($i = 0; $i < 7;$i++){
                ...
                $getSlots = Schedule::where('teacherId',$req->teacherId)->where('date',$date)/*->where('available',1)*/->first();
                $daysData[] = [
                    'date' => $date,
                    'day' => $day,
    ...
    public function createAndUpdateQuestion(Request $req)
    {
        ...
        if(!$validator->fails()){
            ...
            if($status){
                ...
                if($category){
                    $chapter = Chapter::where('id',$req->chapters)->first();
                    if($chapter){
                        $subchapter = SubChapter::where('id',$req->subchapter)->first();
                        if($subchapter){
                            ...
                            $question->question_title = ($req->question_title) ? $req->question_title : '';
    ...
    public function getQuestionList(Request $req)
    {
        ...
        if(!$validator->fails()){
            $chapter = Chapter::where('id',$req->chapterId)->first();
            if($chapter){
                $question = Question::select('*')->with('chapter')->with('category')->with('course')->with('subchapter');
                if(!empty($req->categoryId)){
                ...
                if(!empty($req->courseId)){
                    $question = $question->where('courseId',$req->courseId);
                }
    ...
    {
        $commonQuestion = CommonQuestion::where('membership_id', 0)->get();
        return sendResponse('MemberShip List',$commonQuestion);
    }

    public function getUserMemberShip(Request $req)
    {
        $memberShip = BuyMemberShip::select('*')->with('membership')->with('transactionDetails')->get();
        if(!empty($req->userId)){
            $memberShip2 = $memberShip->where('userId',$req->userId)->where('userType',$req->userType);
        }
        // if(!empty($req->userType)){
        //     $memberShip2 = $memberShip->where('userType',$req->userType);
        // }
        // $memberShip = $memberShip->get();
        return sendResponse('MemberShip List of The User',$memberShip2);
    }
    ...
    public function bookTheSlot(Request $req)
    {
        ...
        if(!$validator->fails()){
            ...
            try {
                ...
                $booking->slot_time = $slot->time;
                $booking->slot_day = $slot->day;
                $booking->zoom_id = $slot->credit;
                $booking->save();
                $slot->available = 2;
                $slot->save();
                $zoom = $this->createZoomMeeting($slot,$req->userId,$req->userType);
                $tecaherBooking = TeacherBooking::where('id', $booking->id)->first();
                $tecaherBooking->zoom_id = $zoom->id;
                $tecaherBooking->save();
                $data = [
                    'Schedule'  =>  $slot,
                    'booking'   =>  $tecaherBooking,
                    'zoom'      =>  $zoom,
                    'slotTime'  =>  date('Y-m-d',strtotime($slot->date)).'T'.date($slot->time)
                ];
                DB::commit();
    ...
    public function createZoomMeeting($slotData, $userId, $userType)
    {
        $getTeacherDetails = Teacher::where('id', $slotData["teacherId"])->first();
        // return sendResponse('Zoom Meeting Created Success', ["slotData" => $slotData, "userType" => $userType]);
        $topic = 'Meeting with '.$getTeacherDetails->name.' at '.$slotData["date"].' '.$slotData["time"];
        $startTime = date('Y-m-d',strtotime($slotData["date"])).'T'.date($slotData["time"]);
    ...
    public function bookMembership(Request $req)
    {
        ...
        if(!$validator->fails()){
            ...
            //user data
            $userData = User::findOrFail($req->userId);
            $userData->membership_id = $membership->id;
            $userData->save();
            ...
            //user credit list
            $userCreditList = Creditlist::where('user_id',$req->userId)->first();
            if ($userCreditList) {
                $userCreditList->user_id = $req->userId;
                $userCreditList->amount = $userCreditList->amount + $membership->credit;
                $userCreditList->save();
            } else {
                $userCreditList = new Creditlist();
                $userCreditList->user_id = $req->userId;
                $userCreditList->amount = $membership->credit;
                $userCreditList->save();
            }
