...
function fetch_course_details(){
    ...
    if (mysqli_num_rows($result)) {
        ...
        $course['course_lectures'] = fetch_user_course_lectures($user_id,$row['id']);
    }
...
function fetch_user_course_lectures($user_id,$course_id){
    $sql = "select * from course_chapters where courseId='$course_id'";
    $result = mysqli_query(conn(), $sql);
    ...
    if (mysqli_num_rows($result)) {
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            $chapter_id = $row['id'];
            $sql1 = "select * from chapter_purchases where courseId='$course_id' and chapterId='$chapter_id' and userId='$user_id'";
            $result1 = mysqli_query(conn(), $sql1);

            if (mysqli_num_rows($result1)>0) {
                $data['course_permission'] = '1';
            }else{
                $data['course_permission'] = '0';
            }

            array_push($course_lectures, $data);
        }
    }
...
function fetch_teacher_courses($teacher_id){
    $sql = "select * from courses where teacherId='$teacher_id'";
    $result = mysqli_query(conn(), $sql);
...
function purchase_course(){
    ...
    if (mysqli_num_rows($result)) {
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            $sql1 = "insert into chapter_purchases
                    (userId, chapterId, courseId, price, stripeTransactionId, created_at, updated_at)
                    values
                    ('$user_id','$chapter_id','$course_id','$price','$transaction_id','$created_at','$created_at')";
    ...
    $notification = 'You have successfully purchased all the chapters of '.$course_name.'. Now you have the access to all the contents of this course.';
    ...
    die(json_encode(array("error"=>false,"message"=>"You have successfully purchased this course")));
}

function fetch_subscribed_chapters(){
    $user_id = $_GET['user_id'];

    $sql = "select course_chapters.*, courses.course_name
from course_chapters join courses
on (course_chapters.courseId=courses.id)
where course_chapters.id IN
(select chapterId from chapter_purchases where chapter_purchases.userId='$user_id')";

    $result = mysqli_query(conn(), $sql);

    $subscribed_chapters = array();
    if (mysqli_num_rows($result)) {
        while ($row = mysqli_fetch_assoc($result)) {

            array_push($subscribed_chapters, $row);
        }
    }

    die(json_encode(array("error"=>false,"subscribed_chapters"=>$subscribed_chapters)));
}
?>